...
#ifdef OPLUS_BUG_STABILITY
/* Zhijun.Ye@MM.Display.LCD, 2021/11/24, add for bl debug */
static int bl_after_aal = 0;
extern struct timeval end;
/* Chenjun@MM.Display.LCD, 2021/12/22, add for 10239 level brightness */
		...
		int level, int mappingLevel)
{
	...
	/* Zhijun.Ye@MM.Display.LCD, 2021/11/24, modify for bl debug */
...
static int mtk_leds_probe(struct platform_device *pdev)
{
	...
	#ifdef OPLUS_BUG_STABILITY
	#ifdef CONFIG_DRM_MEDIATEK
	/* Zhijun.Ye@MM.Display.LCD, 2022/03/16, add for backlight */
	if (oplus_disp_no_bl_on_power_off) {
